    ...
    public function subscribe_user($list_id, $email_address, $mappingUserDataEntity, $custom_fields = [], $tags = '', $old_email_address = '')
    {
        try {
            ...
            if (isset($response['body']->contact->id)) {
                ...
                if (isset($GLOBALS['fusewp_sync_destination'][$list_id]['account'])) {
            ...
            fusewp_log_error($this->activecampaignInstance->id, __METHOD__ . ':' . is_string($response) ? $response : wp_json_encode($response));
